...
namespace onnx_xla  {
  ...
  onnxStatus translateReshape(const Node& n, XlaBuilder& builder, ValueOpMap& valueToOp, const ValueLiteralMap& valueToLiteral)  {
    ...
    for (std::vector<Dimension>::const_iterator it = originalShape.begin(); it != originalShape.end(); ++it)  {
      if (!it->is_int)  {
        std::cerr << "Reshape operator input data shape is not known" << std::endl;
