...
onnxStatus translateMaxPool(const Node& n,
  ...
  std::vector<int64> windowDimensions(numAxes + 2, 1);
  std::copy(kernelShape.begin(), kernelShape.end(),
            windowDimensions.begin() + 2);

  // Set windowStrides
  std::vector<int64> windowStrides(numAxes + 2, 1);
  ...
  auto poolOp = builder.ReduceWindowWithGeneralPadding(
      inputOp, initValue, max, windowDimensions, windowStrides, padding);
  valueToOp[n.outputs().at(0)] = poolOp;
